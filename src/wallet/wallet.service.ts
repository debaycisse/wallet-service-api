...
export class WalletService {
    ...
    try {
      const response = await axios.post(
        'https://api.paystack.co/transaction/initialize',
        {
          ...
          reference,
          callback_url: `${appUrl}/wallet/deposit/${reference}/status`,
        },
        {
          headers: {
  ...
  async handlePaystackWebhook(signature: string, payload: any) {
    ...
    if (payload.event === 'charge.success') {
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        // Credit wallet
        transaction.wallet.balance = parseFloat(
          transaction.wallet.balance.toString()) + amount;
  ...
  async transfer(userId: string, transferDto: TransferDto) {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      recipientWallet.balance = parseFloat(
        recipientWallet.balance.toString()
      ) + transferDto.amount;
  ...
  async getTransactions(
    userId: string, query: { page?: number; limit?: number }
  ) {
    const wallet = await this.getWalletByUserId(userId);
